#include "Graphics.h"

#include "Window.h"
#include "GraphicsDriverD3D11.h"
#include "Camera.h"
#include "Mesh.h"
#include "ColorShader.h"
#include "LightShader.h"
#include "TextureShader.h"
#include "Light.h"
#include "Bitmap.h"
#include "RenderTexture.h"

Graphics::Graphics() : _window(NULL), _driver(NULL), _camera(NULL), _bitmap(NULL), _mesh(NULL), _textureShader(NULL), _dirLight(NULL), _renderTexture(NULL)
{
}

Graphics::Graphics(const Graphics&)
{
}

Graphics::~Graphics()
{
}

bool Graphics::Initialize(char *appName, int width, int height, bool fullScreen, bool vSync)
{
	_window = new Window();
	if (!_window->InitializeWindow(appName, width, height, fullScreen))
		return false;

	//init d3d11
	_driver = new GraphicsDriverD3D11();
	if (!_driver->Initialize(_window->GetHWND(), width, height, fullScreen, vSync))
		return false;

	_camera = new Camera();
	
	_mesh = new Mesh();
	if (!_mesh->Initialize(_driver->GetD3D11Device(), "Cube.txt", "seafloor.dds"))
		return false;

	_bitmap = new Bitmap();
	if (!_bitmap->Initialize(_driver->GetD3D11Device(), "seafloor.dds", 80, 60))
		return false;
	_bitmap->SetPos(100, 100);

	_textureShader = new TextureShader();
	if (!_textureShader->Initialize(_driver->GetD3D11Device(), _window->GetHWND()))
		return false;

	_dirLight = new Light(DirectX::XMFLOAT4(0.5f, 0.5f, 0.5f, 1.0f),
		DirectX::XMFLOAT4(1.0f, 0.0f, 1.0f, 1.0f), 0.5,
		DirectX::XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f), 32, 0.5,
		DirectX::XMFLOAT3(0.0f, 0.0f, 1.0f));

	_renderTexture = new RenderTexture();
	if (!_renderTexture->Initialize(_driver->GetD3D11Device(), 800, 600))
		return false;

	return true;
}

void Graphics::Shutdown()
{
	if (_renderTexture)
	{
		_renderTexture->Shutdown();
		_renderTexture = NULL;
	}
	if (_dirLight)
	{
		delete _dirLight;
		_dirLight = NULL;
	}
	if (_textureShader)
	{
		_textureShader->Shutdown();
		delete _textureShader;
		_textureShader = NULL;
	}
	if (_bitmap)
	{
		_bitmap->Shutdown();
		delete _bitmap;
		_bitmap = NULL;
	}
	if (_mesh)
	{
		_mesh->Shutdown();
		delete _mesh;
		_mesh = NULL;
	}
	if (_camera)
	{
		delete _camera;
		_camera = NULL;
	}

	if (_driver)
	{
		_driver->Shutdown();
		delete _driver;
		_driver = NULL;
	}

	if (_window)
	{
		_window->ShutdownWindow();
		delete _window;
		_window = NULL;
	}
}

bool Graphics::RenderOneFrame()
{
	if (!_window->ReceiveMessage())
		return false;
	
	if (!Render())
		return false;

	return true;
}

bool Graphics::Render()
{
	_renderTexture->SetRenderTarget(_driver->GetD3D11DeviceContext(), _driver->GetD3D11DepthStencilView());
	_renderTexture->ClearRenderTarget(_driver->GetD3D11DeviceContext(), _driver->GetD3D11DepthStencilView(), 0, 0, 1, 1);
	RenderScene();

	_driver->SetBackBufferRenderTarget();

	_driver->BeginScene(0.f, 0.f, 0.f, 1.f);

	_camera->Render();
	DirectX::XMMATRIX viewMatrix = _camera->GetViewMatrix();

	DirectX::XMMATRIX worldMatrix = _driver->GetWorldMatrix();
	DirectX::XMMATRIX projectionMatrix = _driver->GetProjectionMatrix();
	DirectX::XMMATRIX orthoMatrix = _driver->GetOrthoMatrix();

	_driver->SwitchZBuffer(false);
	_bitmap->Render(_driver->GetD3D11DeviceContext());
	_textureShader->Render(_driver->GetD3D11DeviceContext(), _bitmap->GetIndexCount(), worldMatrix, viewMatrix, orthoMatrix, _renderTexture->GetRenderTargetTextureView());
	_driver->SwitchZBuffer(true);
	RenderScene();
	_driver->EndScene();

	return true;
}

bool Graphics::RenderScene()
{
	_camera->Render();
	DirectX::XMMATRIX viewMatrix = _camera->GetViewMatrix();

	DirectX::XMMATRIX worldMatrix = _driver->GetWorldMatrix();
	DirectX::XMMATRIX projectionMatrix = _driver->GetProjectionMatrix();
	DirectX::XMMATRIX orthoMatrix = _driver->GetOrthoMatrix();

	_mesh->Render(_driver->GetD3D11DeviceContext());
	_textureShader->Render(_driver->GetD3D11DeviceContext(), _mesh->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, _mesh->GetTextureView());

	return true;
}