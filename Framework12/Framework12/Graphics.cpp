...
Graphics::Graphics() : _window(NULL), _driver(NULL), _camera(NULL), _mesh(NULL), _meshList(NULL), _lightShader(NULL), _bitmap(NULL), _textureShader(NULL), _dirLight(NULL), _fontShader(NULL), _text(NULL)
{
}
...
bool Graphics::Initialize(char *appName, int width, int height, bool fullScreen, bool vSync)
{
	...
	_mesh = new Mesh();
	if (!_mesh->Initialize(_driver->GetD3D11Device(), "sphere.txt", "seafloor.dds"))
		return false;
...
bool Graphics::Render()
{
	...
	int meshNum = _meshList->GetMeshNum();
	int renderNum = 0;
	for (int i = 0; i < meshNum; ++i)
	{
		float x, y, z;
		_meshList->GetMeshPos(i, x, y, z);
		bool render = _camera->CheckSphere(x, y, z, 1.0f);
		if (render)
		{
			DirectX::XMMATRIX translateMatrix = DirectX::XMMatrixTranslation(x, y, z);
			worldMatrix = DirectX::XMMatrixMultiply(worldMatrix, translateMatrix);
			_mesh->Render(_driver->GetD3D11DeviceContext());
			_lightShader->Render(_driver->GetD3D11DeviceContext(), _mesh->GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, _mesh->GetTextureView(), _dirLight, _camera->GetPosition());
			worldMatrix = _driver->GetWorldMatrix();
			++renderNum;
		}
	}
	...
	oss << input->GetMouseX() << "  " << input->GetMouseY() << "  " << fps->GetFps() << "  " << fps->GetCpuUsage() << "  " << renderNum;
